...
#include "G01CameraMode_ThirdPerson.h"
#include "Curves/CurveVector.h"
#include "GameFramework/Character.h"
#include "G01CameraAssistInterface.h"
#include "Misc/Optional.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(G01CameraMode_ThirdPerson)

namespace G01CameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UG01CameraMode_ThirdPerson::UG01CameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FG01PenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FG01PenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FG01PenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FG01PenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FG01PenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FG01PenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FG01PenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));

}

void UG01CameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UG01CameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UG01CameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IG01CameraAssistInterface* TargetControllerAssist = Cast<IG01CameraAssistInterface>(TargetController);

	IG01CameraAssistInterface* TargetActorAssist = Cast<IG01CameraAssistInterface>(TargetActor);
	...
	IG01CameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IG01CameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IG01CameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IG01CameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UG01CameraMode_ThirdPerson::PreventCameraPenetration(AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
	...
	{
		FG01PenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(G01CameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UG01CameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
	...
	DisplayDebugManager.DrawString(FString::Printf(TEXT("   Location: %s"), *View.Location.ToCompactString()));
	DisplayDebugManager.DrawString(FString::Printf(TEXT("   Rotation: %s"), *View.Rotation.ToCompactString()));
	DisplayDebugManager.DrawString(FString::Printf(TEXT("   ControlRotation: %s"), *View.ControlRotation.ToCompactString()));
	DisplayDebugManager.DrawString(FString::Printf(TEXT("   FieldOfView: %f"), View.FieldOfView));
...
void UG01CameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UG01CameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
