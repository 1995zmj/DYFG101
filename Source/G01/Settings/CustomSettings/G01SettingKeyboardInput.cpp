...
#include UE_INLINE_GENERATED_CPP_BY_NAME(G01SettingKeyboardInput)

class ULocalPlayer;

#define LOCTEXT_NAMESPACE "LyraSettings"

namespace G01::ErrorMessages
{
	static const FText UnknownMappingName = LOCTEXT("LyraErrors_UnknownMappingName", "Unknown Mapping");
}


UG01SettingKeyboardInput::UG01SettingKeyboardInput()
{
	bReportAnalytics = false;
}

FText UG01SettingKeyboardInput::GetSettingDisplayName() const
{
	...
	return G01::ErrorMessages::UnknownMappingName;
}

FText UG01SettingKeyboardInput::GetSettingDisplayCategory() const
{
	if (const FKeyMappingRow* Row = FindKeyMappingRow())
	{
		if (Row->HasAnyMappings())
...
const FKeyMappingRow* UG01SettingKeyboardInput::FindKeyMappingRow() const
{
...
UEnhancedPlayerMappableKeyProfile* UG01SettingKeyboardInput::FindMappableKeyProfile() const
{
...
UEnhancedInputUserSettings* UG01SettingKeyboardInput::GetUserSettings() const
{
	if (UG01LocalPlayer* G01LocalPlayer = Cast<UG01LocalPlayer>(LocalPlayer))
	{
		// Map the key to the player key profile
		if (UEnhancedInputLocalPlayerSubsystem* System = G01LocalPlayer->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
		{
...
void UG01SettingKeyboardInput::OnInitialized()
{
...
void UG01SettingKeyboardInput::InitializeInputData(const UEnhancedPlayerMappableKeyProfile* KeyProfile, const FKeyMappingRow& MappingData, const FPlayerMappableKeyQueryOptions& InQueryOptions)
{
	check(KeyProfile);
...
FText UG01SettingKeyboardInput::GetKeyTextFromSlot(const EPlayerMappableKeySlot InSlot) const
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
...
void UG01SettingKeyboardInput::ResetToDefault()
{
	if (UEnhancedInputUserSettings* Settings = GetUserSettings())
	{
...
void UG01SettingKeyboardInput::StoreInitial()
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
		if (const FKeyMappingRow* Row = FindKeyMappingRow())
...
void UG01SettingKeyboardInput::RestoreToInitial()
{
...
bool UG01SettingKeyboardInput::ChangeBinding(int32 InKeyBindSlot, FKey NewKey)
{
...
void UG01SettingKeyboardInput::GetAllMappedActionsFromKey(int32 InKeyBindSlot, FKey Key, TArray<FName>& OutActionNames) const
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
...
bool UG01SettingKeyboardInput::IsMappingCustomized() const
{
	bool bResult = false;
